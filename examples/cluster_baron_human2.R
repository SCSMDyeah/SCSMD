### DATA
# human2
h2 <- read.csv(".../baron/GSM2230758_human2_umifm_counts.csv", header = T)
rownames(h2) <- h2[,1]
...
h_ann2 <- data.frame(
  ...
  cell_type1 = c(labels_h2))
rownames(h_ann2) <- colnames(h2)


### SINGLECELLEXPERIMENT
source(".../create_sce.R")
h2_sceset <- create_sce_from_counts(h2, h_ann2)

saveRDS(h2_sceset, "baron-human2.rds")

count_baron_human2 <-readRDS(".../baron/baron-human2.rds")

data<-assay(count_baron_human2)

label <- as.numeric(factor(count_baron_human2$cell_type1))
...
sc<-specc(distance_cell,centers=7, kernel ="rbfdot", kpar = "automatic",nystrom.red = FALSE, nystrom.sample =dim(x)[1]/6, iterations = 200,mod.sample = 0.75, na.action =na.omit)
label_sc<-sc@.Data
ARI_baron_human2 <- adjustedRandIndex(label,as.vector(label_sc))
ARI_result[[i]] <- ARI_baron_human2
...
ARI_baron_human2 <-getmode(ARI_result_vector)


baron_human2_ARI_overall <- c(ARI_sc3, ARI_Seurat, ARI_SHARP, ARI_cidr, ARI_SINCERA, ARI_Rphenograph, ARI_RaceID,ARI_baron_human2)
baron_human2_ARI_overall
baron_human2_overall_rank <-rank(baron_human2_ARI_overall)
baron_human2_overall_rank
