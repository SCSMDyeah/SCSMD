...
count_Goolam <- read.table(".../Goolam/Goolam_et_al_2015_count_table.tsv")
counts_matrix <- data.frame(count_Goolam)

counts_matrix <- as.matrix(counts_matrix) # must be a matrix object!
pretend.cell.labels <- as.factor(rep(c(2,4,8,16,32,8,2,4),c(6,40,16,6,6,16,10,24)))
pretend.gene.lengths <- as.factor(rownames(counts_matrix))

d_Goolam <- SingleCellExperiment(assays = list(
...
data <- assay(d_Goolam)
label <- as.numeric(factor(d_Goolam$cell_type1))
...
for (i in 1:200){
  ...
  label_sc<-sc@.Data
  ARI_Goolam <- adjustedRandIndex(label,as.vector(label_sc))
  ARI_result[[i]] <- ARI_Goolam
...
ARI_Goolam <-getmode(ARI_result_vector)

ARI_Goolam
Goolam_ARI_overall <- c(ARI_sc3, ARI_Seurat, ARI_SHARP, ARI_cidr, ARI_SINCERA, ARI_Rphenograph, ARI_RaceID,ARI_Goolam)
Goolam_ARI_overall
Goolam_overall_rank <-rank(Goolam_ARI_overall)
Goolam_overall_rank
