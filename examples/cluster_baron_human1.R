### DATA
# human1
h1 <- read.csv(".../baron/GSM2230757_human1_umifm_counts.csv", header = T)
rownames(h1) <- h1[,1]
...
h_ann1 <- data.frame(
  ...
  cell_type1 = c(labels_h1))
rownames(h_ann1) <- colnames(h1)


### SINGLECELLEXPERIMENT
source(".../create_sce.R")
h1_sceset <- create_sce_from_counts(h1, h_ann1)
...
count_baron_human1 <-readRDS(".../baron/baron-human1.rds")

data<-assay(count_baron_human1)

label <- as.numeric(factor(count_baron_human1$cell_type1))
...
label_sc<-sc@.Data
ARI_baron_human1 <- adjustedRandIndex(label,as.vector(label_sc))
ARI_result[[i]] <- ARI_baron_human1
label_result[[i]] <- label_sc
}

ARI_result_vector <- as.vector(unlist(ARI_result))

ARI_baron_human1 <-getmode(ARI_result_vector)

baron_human1_ARI_overall <- c(ARI_sc3, ARI_Seurat, ARI_SHARP, ARI_cidr, ARI_SINCERA, ARI_Rphenograph, ARI_RaceID,ARI_baron_human1)
baron_human1_ARI_overall
baron_human1_overall_rank <-rank(baron_human1_ARI_overall)
baron_human1_overall_rank
